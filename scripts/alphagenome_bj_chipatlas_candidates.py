#!/usr/bin/env python3
"""AlphaGenome BJ-context annotation for ChIP-Atlas TERF1-overlap DEGs."""
...
CANDIDATES = ROOT / "results/new_data/chipatlas/chipatlas_terf1_deg_overlap.tsv"
...
def load_candidate_genes() -> list[str]:
    ...
    # The 10 kb file is the superset of the three ChIP-Atlas windows.
    genes = candidates.loc[candidates["window"] == "10kb", "gene_name"].dropna().unique()
    return sorted(str(g).strip() for g in genes if str(g).strip())


def load_loci(genes: list[str]) -> pd.DataFrame:
    ...
    ann = ann[ann["gene_name"].isin(genes)].copy()
    ...
    ann = (
        ann.sort_values(["gene_name", "chr", "gene_start", "gene_end"])
        .drop_duplicates("gene_name")
...
def summarize_track(
    gene_row: pd.Series, interval: genome.Interval, attr: str, track
...
def main() -> None:
    ...
    loci = load_loci(genes)
    loci.to_csv(OUTDIR / "alphagenome_bj_chipatlas_candidate_loci.tsv", sep="\t", index=False)
    ...
    for _, gene_row in loci.iterrows():
        ...
        print(f"Predicting {gene_row['gene_name']} at {interval}", flush=True)
        try:
            ...
            print(f"Failed {gene_row['gene_name']}: {type(exc).__name__}", flush=True)
            continue
        for attr, _output_type in OUTPUT_ATTRS:
            rows.extend(summarize_track(gene_row, interval, attr, getattr(output, attr)))
        print(f"Finished {gene_row['gene_name']}", flush=True)

        pd.DataFrame(rows).to_csv(
            OUTDIR / "alphagenome_bj_chipatlas_candidate_track_summary.tsv",
            sep="\t",
            index=False,
        )
    ...
    compact.to_csv(
        OUTDIR / "alphagenome_bj_chipatlas_candidate_compact_summary.tsv",
        sep="\t",
        index=False,
    )

    lines = [
        "# AlphaGenome BJ annotation for ChIP-Atlas TERF1-overlap DEGs",
        "",
        "Input: DEGs overlapping ChIP-Atlas hg38 TERF1 target-gene table at TSS +/- 10 kb.",
        "This uses 131,072 bp TSS-centered hg38 intervals and BJ ontology `EFO:0002779`.",
        "",
        f"- Candidate genes requested: {', '.join(genes)}",
    ...
    lines.extend(
        [
            "Interpretation rule: AlphaGenome has no TERF1/TRF1 ChIP-TF track in",
            "the inspected human metadata, so these outputs annotate basal BJ-like",
            "regulatory context only. They do not test direct TRF1 binding or",
            "siTRF1 perturbation causality.",
            "",
        ]
    )
    (OUTDIR / "alphagenome_bj_chipatlas_candidate_summary.md").write_text("\n".join(lines))
    print("\n".join(lines))
